...
### Admin 고도화 (2026-04-08)

| 결정 | 내용 |
|------|------|
| 쓰기 권한 | 운영 기능(타 사용자 프로필 수정, 문의 답변, 공지 CRUD, 계정 비활성)은 **RLS로 관리자 세션에만** 허용. 서비스 롤은 관리자 웹에 넣지 않음. |
| `is_admin` 변경 | API 세션(`auth.uid()` 존재)에서는 **`is_admin` 컬럼 UPDATE 불가** 트리거 + **INSERT 시 항상 false** 트리거. 승격/해제는 SQL Editor 등 `auth.uid()` 없는 경로만. |
| 계정 비활성 | `profiles.account_disabled` 플래그. **본인은 이 필드를 바꿀 수 없음**(트리거). 앱·관리자 웹 모두 프로필에서 true면 **세션 종료**(관리자 UI 접근 차단). Auth `ban` API는 본 스코프에서 쓰지 않음(anon만 사용). |
| 문의 | `support_tickets` 테이블. 사용자는 **본인 행만 INSERT/SELECT**, 관리자는 전체 SELECT·UPDATE(상태·답변). |
| 공지 | `app_notices` 테이블. 로그인 사용자는 **노출 기간·활성 조건**을 만족하는 행만 SELECT, 관리자는 CRUD 전부. |
