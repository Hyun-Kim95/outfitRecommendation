...
`project` 슬러그는 `.obsidian-ingest.json`의 `slug`가 있으면 그 값을, 없으면 Git 루트 폴더명을 쓴다. **선택 필드 `displayName`**이 있으면 볼트 허브 노트(`_project-doc-index`) 제목·YAML `display_name`, 커밋 저널, `normalize-doc-frontmatter`로 붙는 lane 문서에도 반영되어 Dataview에서 사람이 읽기 쉬운 이름으로 구분할 수 있다. Vault 링크 표시 텍스트는 Windows 콘솔 인코딩 호환을 위해 ASCII로 둔다.
