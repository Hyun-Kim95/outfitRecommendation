...
`project` 슬러그는 `.obsidian-ingest.json`의 `slug`가 있으면 그 값을, 없으면 Git 루트 폴더명을 쓴다. **선택 필드 `displayName`·`hubFileStem`**은 허브 파일명·제목·`display_name`·커밋 저널·lane 문서의 Vault 링크에 반영된다(규칙: `scripts/obsidian/Resolve-HubIndexStem.ps1`). Vault 링크 표시 텍스트는 Windows 콘솔 인코딩 호환을 위해 ASCII로 둔다.
