...
기준: [AGENTS.md](../../AGENTS.md), [.cursor/rules/60-delivery-gates.mdc](../../.cursor/rules/60-delivery-gates.mdc).

## 필수 항목
- [x] PRD가 존재하고 실제 구현 범위와 일치한다 — [prd.md](../requirements/prd.md) 존재(정적); 최신 기능 커밋 시 재대조
- [x] 프론트엔드/백엔드 계획 문서가 존재한다 — [frontend-plan.md](../requirements/frontend-plan.md), [backend-plan.md](../requirements/backend-plan.md)
- [x] UI 변경이 있었다면 UI 명세가 업데이트되었다 — [ui-spec.md](../design/ui-spec.md); 관리자 고도화·모바일 문의 반영됨(2026-04-08)
...
- [x] API/스키마 변경 사항이 문서화되었다 — [backend-plan.md](../requirements/backend-plan.md), `supabase/migrations/20260408210000_admin_inquiries_notices_account_disabled.sql`
