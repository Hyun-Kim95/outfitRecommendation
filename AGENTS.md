...
세부 정책은 `.cursor/rules/`, 작업 절차는 기본적으로 User-level skills(로컬에는 `client-project-lifecycle` 중심)를 따른다. `.cursor/agents/`가 없는 환경에서는 메인 에이전트가 동일 역할 범위를 직접 수행한다.
...
- 본 레포처럼 PRD·화면 스펙·기본 계약 문서가 이미 존재하는 운영 단계 프로젝트는 기본적으로 `bugfix-flow` 또는 `start-feature`를 우선 적용한다.
- 사용자가 명시적으로 고객사 전체 프로젝트(엔드투엔드)를 처음부터 진행하라고 요청한 경우에만 `client-project-lifecycle` 기본 진입을 강제한다.
