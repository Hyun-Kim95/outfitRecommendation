...
- 모든 작업은 착수 전에 **실행 계획**을 먼저 제시하고, 계획 형식·Owner 표기·통합 책임·재계획 트리거는 `.cursor/rules/75-plan-and-delegation.mdc`를 단일 기준으로 따른다.
- **직접 처리 가능한 예외** 또는 사용자의 **라이트만/계획 축약** 지시가 있으면 `.cursor/rules/75-plan-and-delegation.mdc`의 라이트 템플릿을 적용한다.
- 사용자 입력의 기본 해석(문장형 1~2문장), 결과 보고 형식, 승인 대기 표기는 `.cursor/rules/55-output-contract.mdc`를 우선 적용한다.
