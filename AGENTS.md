...
3. `.cursor/rules/70-client-lifecycle-default.mdc`가 **적용되는 고객 전체 납품 대화**에서만, 그에 따른 **HUMAN·승인·멈춤** 구간(스킬 정의)
...
## 본 레포 전제 (진행 중·개인용 제품)
- 본 레포는 **이미 PRD·화면 스펙 등 Gate 1에 해당하는 산출물이 존재하는 진행 중 제품**(개인용 앱·관리자 웹)으로 본다.
- **내부·개인 유지보수·소규모 수정**에는 `client-project-lifecycle`을 기본으로 강제하지 않는다. (해당 스킬은 고객사 **신규 전체 납품** 대화용.)
- **신규 대외 기능**, **새 대외 API·계약**, 사용자가 **전체 납품·고객 프로젝트 파이프라인**을 명시한 경우에만 게이트·납품 스킬을 엄격히 적용한다.
