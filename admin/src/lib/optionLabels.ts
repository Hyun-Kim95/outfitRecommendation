...
const KO_TO_EN: Record<string, string> = {
  ...
  추가: 'More',
  // default transports (profile; 버스·기타 등은 위 카테고리와 동일 키 공유)
  도보: 'Walking',
  지하철: 'Subway',
  자가용: 'Car',
  자전거: 'Bicycle',
...
/** 쉼표 또는 앱 저장 형식인 ` · `로 이어진 카테고리 문자열을 토큰으로 분리 */
...
/**
 * 일부 옵션은 한 개 라벨인데 저장/표시 시 `조거 · 트레이닝`처럼 가운뎃점으로만 나뉘어
 * 두 토큰이 됨 → 합쳐서 `KO_TO_EN`의 복합 키와 맞춤.
 */
...
/** 문자열 배열·단일 문자열은 옵션 라벨로, 그 외는 JSON 문자열 */
export function formatOptionArrayOrJson(locale: Locale, v: unknown): string {
  if (v == null) return '—';
  if (Array.isArray(v)) {
    if (v.length === 0) return '—';
    if (v.every((x) => typeof x === 'string')) {
      return (v as string[]).map((x) => optionListLabel(locale, x)).join(', ');
    }
  }
  if (typeof v === 'string') return optionLabel(locale, v);
  try {
